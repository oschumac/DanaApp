...
//    for (byte element : content) {
  ...
  public static void validateCrc(short receivedCrc, byte[] content, int offset, int length) {

	  short expectedCrc = getCrc16(content, offset, length);

    if (receivedCrc != expectedCrc) {
      throw new IllegalStateException("Invalid crc, expected "+Short.toString(expectedCrc)+" received "+
               Short.toString(receivedCrc));
    }
  }
